...
```
📁 项目文件夹
  ├─ 📄 README.md        2025/11/5 13:42, 15.2 kB  [📋]
  ├─ 📄 package.json     2025/11/5 13:40, 10.8 kB  [📋]
  └─ 📁 src/
      ├─ 📄 extension.ts  2025/11/5 12:30, 45.6 kB  [📋]
      └─ 📁 utils/
          └─ 📄 helper.ts  2025/11/4 18:20, 8.3 kB   [📋]
```
...
```json
{
  "fileInfoDecorator.showSize": true,           // 显示文件大小
  "fileInfoDecorator.showDate": true,           // 显示修改时间
  "fileInfoDecorator.dateFormat": "YYYY/M/D HH:mm",  // 日期格式
  "fileInfoDecorator.showOnFolders": false      // 文件夹显示信息
}
```
...
```json
{
  "fileInfoDecorator.useRelativePathForChat": false,  // Chat使用绝对路径
  "fileInfoDecorator.useRelativePathOnDrop": true     // 拖放使用相对路径
}
```
...
1. **node_modules等大型目录** - 为性能考虑，索引时会跳过这些目录
2. **选中样式** - 某些主题的选中颜色可能不够明显（可通过设置调整）
...
- [INSTALL - 安装说明](https://github.com/Qinsheng-Lin/xxh-file-detail/blob/main/INSTALL.md)
